...
- Regular file bytes, executable bits where the filesystem supports them, and symlink target text/types are checked against the staged Git blobs before committing. Symlinks are copied without following them. On Windows, creating/restoring symlinks requires Developer Mode or suitable privileges.
